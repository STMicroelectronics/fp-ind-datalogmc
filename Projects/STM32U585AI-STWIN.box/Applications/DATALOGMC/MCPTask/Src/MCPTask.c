...
float_t MCPTask_GetVoltageScaler(void)
{
  return sTaskObj.scaleFlashParams.voltage;
}

float_t MCPTask_GetCurrentScaler(void)
{
  return sTaskObj.scaleFlashParams.current;
}
float_t MCPTask_GetFrequencyScaler(void)
{
...
/**
  * @brief Get the Slow Telemetry Output Data Rate (ODR).
  *
  * This function calculates the slow telemetry ODR based on the
  * MCP_TELEMETRY_REQUEST_TIMER_PERIOD_MS constant.
  *
  * @return The slow telemetry ODR in Hz.
  */
float_t MCPTask_GetSlowTelemetryODR(void)
{
  float_t odr = 0.0f;
  odr = (float_t)((1.0f / (float_t)MCP_TELEMETRY_REQUEST_TIMER_PERIOD_MS) * 1000.0f);
  return odr;
}
