...
/* ASPEP Transport Layer State Machine Type*/
typedef enum
{
  WAITING_PACKET,
  WAITING_PAYLOAD,
...
bool ASPEP_CheckBeacon(ASPEP_Handle_t *pHandle, ASPEP_Capabilities_def *SlaveCapabilities)
{
  ...
  if ((SlaveCapabilities->DATA_CRC != pHandle->Capabilities.DATA_CRC) ||
      (SlaveCapabilities->RX_maxSize != pHandle->Capabilities.RX_maxSize)
      || /* Data packet the master can send is bigger than slave can receive */
      (pHandle->Capabilities.TXS_maxSize != SlaveCapabilities->TXS_maxSize)
      ||  /*Sync packet size alignment is required in order for the master to be able to store it, and to not request a response bigger than slave capability */
      (pHandle->Capabilities.TXA_maxSize != SlaveCapabilities->TXA_maxSize)
      ||  /*Async packet the slave can send is bigger than the master can receive (but master must not request a bigger packet than slave can transmit */
      (SlaveCapabilities->version !=
       pHandle->Capabilities.version))  /* For the time being, master version has to match slave version */
  {
    result = false;
  }
  return result;
      ...
      if (ASPEP_CheckHeaderCRC(*(uint32_t *)aspepHandle.rxHeader) == true)
      {
        ...
        switch (aspepHandle.rxPacketType)
        {
          case response:
            ...
            {
              UASPEP_RECEIVE_BUFFER(&UASPEP_B, aspepHandle.rxBuffer,
                                    aspepHandle.rxLength + ASPEP_DATACRC_SIZE * aspepHandle.Capabilities.DATA_CRC);
          ...
          case async:
            ...
            if (aspepHandle.rxLength <= aspepHandle.txAsyncMaxPayload)
            {
              if (aspepHandle.asyncBufferA.state == freeBuf)
              {
                ...
                UASPEP_RECEIVE_BUFFER(&UASPEP_B, aspepHandle.asyncBufferA.buffer,
                                      aspepHandle.rxLength + ASPEP_DATACRC_SIZE * aspepHandle.Capabilities.DATA_CRC);
                aspepHandle.ASPEP_TL_State = WAITING_PAYLOAD;
              ...
              else if (aspepHandle.asyncBufferB.state == freeBuf)
              {
                ...
                UASPEP_RECEIVE_BUFFER(&UASPEP_B, aspepHandle.asyncBufferB.buffer,
                                      aspepHandle.rxLength + ASPEP_DATACRC_SIZE * aspepHandle.Capabilities.DATA_CRC);
                aspepHandle.ASPEP_TL_State = WAITING_PAYLOAD;
              ...
              else
              {
                ...
                UASPEP_RECEIVE_BUFFER(&UASPEP_B, aspepHandle.LastUsedAsyncBuff->buffer,
                                      aspepHandle.rxLength + ASPEP_DATACRC_SIZE * aspepHandle.Capabilities.DATA_CRC);
                aspepHandle.ASPEP_TL_State = WAITING_PAYLOAD;
  ...
  switch (aspepHandle.CurrentrxPacketType)
  {
    ...
    case ping:
      ...
      else if (aspepHandle.rxPingCbit == configured)
      {
        ...
        {
          aspepHandle.ASPEP_State = ASPEP_TRANSACTION;
          aspepHandle.PacketReady = true; //without payload?
        }
        else   //ping N-1
        {
          aspepHandle.ASPEP_State = ASPEP_TRANSACTION;
...
void USART3_IRQHandler(void)
{
      ...
      && LL_USART_IsEnabledIT_ERROR(USARTB))
  {
    ...
    /* We disable ERROR interrupt to avoid to trig one Overrun IT per additional byte received*/
