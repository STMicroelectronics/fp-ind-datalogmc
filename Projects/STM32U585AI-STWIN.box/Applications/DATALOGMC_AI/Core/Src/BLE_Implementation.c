...
void BLE_InitCustomService(void)
{
  ...
  if (BleManagerAddChar(BLE_InitRawPnPLControlledService(21, 1)) == 0)
  {
    PRINT_DBG("Error adding PnPLike characteristic\r\n");
  }
...
static void BLE_SetCustomStreamID(void)
{
  slow_mc_telemetries_get_stream_id(&customStream.slow_telemetries_id);
  ai_motor_classifier_get_stream_id(&customStream.ai_classifier_id);
}
...
static sys_error_code_t BLE_PostCustomData(int8_t id_stream, uint8_t *buf, uint32_t size)
{
  ...
  if (id_stream == customStream.slow_telemetries_id)
  {
    slow_mc_telemetries_get_st_ble_stream__id(&stream_id);
    customStream.slow_telemetries_buf[0] = stream_id;
    memcpy(&customStream.slow_telemetries_buf[1], buf, size);
    customStream.slow_telemetries_size = size + 1;
    customStream.slow_telemetries_data_ready = 1;
    res = SYS_NO_ERROR_CODE;
  }

  else if (id_stream == customStream.ai_classifier_id)
  {
    ai_motor_classifier_get_st_ble_stream__id(&stream_id);
    customStream.ai_classifier_buf[0] = stream_id;
    memcpy(&customStream.ai_classifier_buf[1], buf, size);
    customStream.ai_classifier_size = size + 1;
    customStream.ai_classifier_data_ready = 1;
    res = SYS_NO_ERROR_CODE;
  }
...
static void BLE_SendCustomData(uint8_t id_stream)
{
  ...
  if (id_stream == customStream.slow_telemetries_id)
  {
    if (customStream.slow_telemetries_data_ready)
    {
      BLE_RawPnPLControlledStatusUpdate(customStream.slow_telemetries_buf, customStream.slow_telemetries_size);
      customStream.slow_telemetries_data_ready = 0;
    }
  }

  else if (id_stream == customStream.ai_classifier_id)
  {
    if (customStream.ai_classifier_data_ready)
    {
      BLE_RawPnPLControlledStatusUpdate(customStream.ai_classifier_buf, customStream.ai_classifier_size);
      customStream.ai_classifier_data_ready = 0;
    }
  }
}
