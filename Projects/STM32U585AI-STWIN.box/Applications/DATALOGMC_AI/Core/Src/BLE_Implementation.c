...
typedef struct _BLE_MCStream_t
{
  uint8_t pnpl_id;

  int8_t slow_telemetries_id;
  ble_notify_event_t slow_telemetries_enabled;
  uint8_t slow_telemetries_buf[21];
  uint8_t slow_telemetries_size;
  uint8_t slow_telemetries_data_ready;

  int8_t ai_classifier_id;
  ble_notify_event_t ai_classifier_enabled;
  uint8_t ai_classifier_buf[6];
  uint8_t ai_classifier_size;
  uint8_t ai_classifier_data_ready;
} BLE_MCStream_t;

BLE_MCStream_t ble_mc_stream;
...
static sys_error_code_t BLE_PostCustomData(int8_t id_stream, uint8_t *buf, uint32_t size);
...
void ble_init_custom_service(void)
{
  ...
  /* Service initialization RawPnPLControlled feature: 1byte for ID + 4byte for each telematries */
  if (ble_manager_add_char(ble_init_raw_pn_pl_controlled_service(21, 1)) == 0)
  {
...
static void BLE_SetCustomStreamID(void)
{
  slow_mc_telemetries_get_stream_id(&ble_mc_stream.slow_telemetries_id);
  ai_motor_classifier_get_stream_id(&ble_mc_stream.ai_classifier_id);
}
...
static sys_error_code_t BLE_PostCustomData(int8_t id_stream, uint8_t *buf, uint32_t size)
{
  ...
  if (id_stream == ble_mc_stream.slow_telemetries_id)
  {
    slow_mc_telemetries_get_st_ble_stream__id(&stream_id);
    ble_mc_stream.slow_telemetries_buf[0] = stream_id;
    memcpy(&ble_mc_stream.slow_telemetries_buf[1], buf, size);
    ble_mc_stream.slow_telemetries_size = size + 1;
    ble_mc_stream.slow_telemetries_data_ready = 1;
    res = SYS_NO_ERROR_CODE;
  }
  else if (id_stream == ble_mc_stream.ai_classifier_id)
  {
    ai_motor_classifier_get_st_ble_stream__id(&stream_id);
    ble_mc_stream.ai_classifier_buf[0] = stream_id;
    memcpy(&ble_mc_stream.ai_classifier_buf[1], buf, size);
    ble_mc_stream.ai_classifier_size = size + 1;
    ble_mc_stream.ai_classifier_data_ready = 1;
    res = SYS_NO_ERROR_CODE;
  }
  return res;
}
...
static void BLE_SendCustomData(uint8_t sId)
{
  if (sId == ble_mc_stream.slow_telemetries_id)
  {
    if (ble_mc_stream.slow_telemetries_data_ready)
    {
      ble_raw_pn_pl_controlled_status_update(ble_mc_stream.slow_telemetries_buf, ble_mc_stream.slow_telemetries_size);
      ble_mc_stream.slow_telemetries_data_ready = 0;
    }
  }
  else if (sId == ble_mc_stream.ai_classifier_id)
  {
    if (ble_mc_stream.ai_classifier_data_ready)
    {
      ble_raw_pn_pl_controlled_status_update(ble_mc_stream.ai_classifier_buf, ble_mc_stream.ai_classifier_size);
      ble_mc_stream.ai_classifier_data_ready = 0;
    }
  }
}
