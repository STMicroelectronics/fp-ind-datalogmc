...
struct _DatalogAppTask
{
  ...
  //** AI Classifier Data Event Listener **/
  IDataEventListener_t AIclassifierListener;
...
//TODO could be more useful to have a CommandParse Class? (ICommandParse + PnPLCommand_t)
  ...
  /** We are using the same structure contex for both sensors
   *  and actuators.
...
typedef struct _DatalogAppTaskClass
{
  ...
  /** AI Classifier Data Event Listener **/
  IDataEventListener_vtbl AIclassifierListenerVTBL;
...
static const DatalogAppTaskClass_t sTheClass =
{
    ...
    {
        DatalogAppTask_AIClassifierListenerOnStatusChange_vtbl,
        DatalogAppTask_AIClassifierListenerSetOwner_vtbl,
        DatalogAppTask_AIClassifierListenerGetOwner_vtbl,
        DatalogAppTask_AIClassifierListenerOnNewDataReady_vtbl },
...
AManagedTaskEx *DatalogAppTaskAlloc()
{
  ...
  sTaskObj.AIclassifierListener.vptr = &sTheClass.AIclassifierListenerVTBL;
...
IEventListener* DatalogAppTask_GetAIClassifierEventListenerIF(DatalogAppTask *_this)
{
  ...
  return (IEventListener*) &_this->AIclassifierListener;
}
                                                 ...
                                                 ULONG *pParams)
{
  ...
  *pAutoStart = TX_AUTO_START;
  ...
  IEventListenerSetOwner((IEventListener *) &p_obj->AIclassifierListener, (void *) p_obj);
...
sys_error_code_t DatalogAppTask_vtblDoEnterPowerMode(AManagedTask *_this, const EPowerMode ActivePowerMode, const EPowerMode NewPowerMode)
{
  ...
  if (ActivePowerMode == E_POWER_MODE_SENSORS_ACTIVE && NewPowerMode == E_POWER_MODE_STATE1)
  {
    ...
    else if (p_obj->datalog_model->log_controller_model.interface == LOG_CTRL_MODE_USB) /*stop command from USB*/
    {
      ...
      {
        if(p_obj->datalog_model->a_models[i] != NULL)
        {
          if(p_obj->datalog_model->a_models[i]->enable)
          {
            stream_id = p_obj->datalog_model->a_models[i]->stream_params.stream_id;
            IStream_dealloc((IStream_t*) p_obj->usbx_device, stream_id);
          }
        }
      }
...
sys_error_code_t DatalogAppTask_ActuatorListenerOnNewDataReady_vtbl(IEventListener *_this, const DataEvent_t *p_evt)
{
  ...
  if(nPMState == E_POWER_MODE_SENSORS_ACTIVE)
  {
    ...
    else
    {
      ...
      {
        /* n_samples_to_timestamp = 0 if user setup spts = 0 (no timestamp needed) */
        if(p_obj->actuatorContext[aId].n_samples_to_timestamp == 0 || samplesToSend < p_obj->actuatorContext[aId].n_samples_to_timestamp)
        {
          if(IStream_is_enabled((IStream_t*) p_obj->usbx_device))
          {
            res = IStream_post_data((IStream_t*) p_obj->usbx_device, stream_id, data_buf, samplesToSend * nBytesPerSample);
...
{
  DatalogAppTask *p_obj = (DatalogAppTask *)((uint32_t) _this - offsetof(DatalogAppTask, AIclassifierListener));
  p_obj->owner = p_owner;
}
...
sys_error_code_t DatalogAppTask_AIClassifierListenerOnNewDataReady_vtbl(IEventListener *_this, const DataEvent_t *p_evt)
{
  ...
  float_t *p_payload_f = (float_t*)EMD_Data(p_evt->p_data);

  uint8_t class = (uint8_t)p_payload_f[1] - 1;
  float_t probabilities = p_payload_f[1+(uint8_t)p_payload_f[1]];

  uint8_t ai_payload_buff_u8[AI_PAYLOAD_BUFF_DIM];

#ifdef SYS_ENABLE_FAKE_AI_CLASSIFIER
  probabilities = 0.7;
  ai_payload_buff_u8[0] = 1;
  memcpy((void*)&ai_payload_buff_u8[1], (void*)&probabilities, 4);
#else
  ai_payload_buff_u8[0] = class;
  memcpy((void*)&ai_payload_buff_u8[1], (void*)&probabilities, 4);
#endif
  ...
  if(nPMState == E_POWER_MODE_SENSORS_ACTIVE)
  {
    ...
    if(IStream_is_enabled((IStream_t*) p_obj->usbx_device))
    {
      res = IStream_post_data((IStream_t*) p_obj->usbx_device, stream_id, (uint8_t *)ai_payload_buff_u8, AI_PAYLOAD_BUFF_DIM);
    }
    if(IStream_is_enabled((IStream_t*) p_obj->ble_device))
    {
      res = IStream_post_data((IStream_t*) p_obj->ble_device, stream_id, (uint8_t *)ai_payload_buff_u8, AI_PAYLOAD_BUFF_DIM);
    }
      ...
      for (i = 0; i < SENSOR_NUMBER; i++)
      {
        if(p_obj->datalog_model->s_models[i] != NULL)
        {
          if(p_obj->datalog_model->s_models[i]->sensor_status->is_active)
          {
            ...
            IStream_alloc_resource((IStream_t*) p_obj->filex_device, stream_id, sd_dps, p_obj->datalog_model->s_models[i]->comp_name);
          }
        }
      }
      ...
      for(i = 0; i < ACTUATOR_NUMBER; i++)
      {
        if(p_obj->datalog_model->ac_models[i] != NULL)
        {
          if(p_obj->datalog_model->ac_models[i]->actuatorStatus.is_active)
          {
            ...
            IStream_alloc_resource((IStream_t*) p_obj->filex_device, stream_id, sd_dps, p_obj->datalog_model->ac_models[i]->comp_name);
          }
        }
      }
    ...
    else if (interface == LOG_CTRL_MODE_USB) /*Start log on USB*/
    {
      ...
      for(i = 0; i < ACTUATOR_NUMBER; i++)
      {
        if(p_obj->datalog_model->ac_models[i] != NULL)
        {
          if(p_obj->datalog_model->ac_models[i]->actuatorStatus.is_active)
          {
            ...
            /** use to set ep**/
            usbx_dctrl_class_set_ep(p_obj->usbx_device, stream_id, usb_ep);
            IStream_alloc_resource((IStream_t*) p_obj->usbx_device, stream_id, usb_dps, p_obj->datalog_model->ac_models[i]->comp_name);
          }
        }
      }
      ...
      for(i = 0; i < ALGORITHM_NUMBER; i++)
      {
        if(p_obj->datalog_model->a_models[i] != NULL)
        {
          ...
          {
            stream_id = p_obj->datalog_model->a_models[i]->stream_params.stream_id;
            usb_ep = p_obj->datalog_model->a_models[i]->stream_params.usb_ep;
            usb_dps = p_obj->datalog_model->a_models[i]->stream_params.usb_dps;
            /** use to set ep**/
            usbx_dctrl_class_set_ep(p_obj->usbx_device, stream_id, usb_ep);
            IStream_alloc_resource((IStream_t*) p_obj->usbx_device, stream_id, usb_dps, p_obj->datalog_model->a_models[i]->comp_name);
          }
        }
      }
